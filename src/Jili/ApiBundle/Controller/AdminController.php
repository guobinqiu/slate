...
//         $response->headers->set("Pragma","no-cache");
    ...
    public function exchangeOK($exchange,$points,$finish_time,$type)
    {
        ...
        if(!$exchange->getStatus()){
            ...
            $this->get('app.latest_news_service')->insertLatestNews(substr($user->getNick(), 0, 3) . '**' . $comment .'兑换' . $points . '积分');
        }
        return true;
    }
    ...
    */
    ...
    public function alipayExchangeOK($exchange_id, $finish_time, $points){
        ...
        if($points_exchange != null){
            ...
            $user = $em->getRepository('WenwenFrontendBundle:User')->find($user_id);
            $this->get('app.latest_news_service')->insertLatestNews(substr($user->getNick(), 0, 3) . '**支付宝兑换' . $points . '积分');
