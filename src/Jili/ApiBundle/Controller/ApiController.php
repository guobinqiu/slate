...
class ApiController extends Controller
{
	...
	/**
	 * @Route("/recoverGamePoint", name="_api_recover_game_point")
	 */
    public function recoverGamePointAction(){

        $request = $this->get('request');
		$do =(int) $request->query->get('do' , 0);
  ...
  //      for($i= 15055; $i<= 15254; $i++) {
            ...
            if(! empty($rs)) {
                $point_uid =$rs->getPointUid();
                $point=$rs->getGamePoint();
                $date=$rs->getGameDate();
                $time=$rs->getGameTime();
                $score=$rs->getGameScore();
                $type=$rs->getGameType();
                $massPoint=$rs->getMassPoint();
                $goalPoint=$rs->getGoalPoint();
                $rankingPoint=$rs->getRankingPoint();
                $attendancePoint=$rs->getAttendancePoint();

                // point_history  里面如果已经有这个user_id ,(  reason= 4 or reason= 5 or reason= 6) 
                $repository = $this->getDoctrine()
                    ->getRepository('JiliApiBundle:PointHistory0'. ($point_uid % 10));

                $qb = $repository->createQueryBuilder('q');
                $qb->where( $qb->expr()->eq('q.userId', $point_uid  )  );
                $qb->andWhere( $qb->expr()->lt('q.createTime', ':p1'));
                $qb->andWhere( $qb->expr()->gte( 'q.createTime' , ':p2'));
                $qb->andWhere( $qb->expr()->in( 'q.reason' ,array(4,5,6) ));
                $qb->setParameter('p1',new \Datetime('2014-01-25 00:00:00') ) ;
                $qb->setParameter('p2',new \Datetime('2014-01-24 00:00:00') ) ;
                $q = $qb->getQuery();
                ...
                if( empty($result) ) {
                    ...
                    if( 1 === $do ) {
                        echo 'game_log: ',$i , ' proccessed' ,'<br />';
                    }
                } else {
                    $ph_delete =  'point_history0'. ( $point_uid % 10);
                    $ids_delete  = array();
                    foreach($result as $row) {
                        $ids_delete[] =  $row->getId();
                    }

                    echo '-- ' , $i,' exit <br />';
                    echo 'Delete from '.$ph_delete .' where id in ('.implode(',', $ids_delete) .') limit '.count( $ids_delete) .'; <br />' ;
                    echo '<br />';
