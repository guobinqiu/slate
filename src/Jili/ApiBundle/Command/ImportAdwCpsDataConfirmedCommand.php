...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while($v = fgetcsv($handle)) {
            ...
            if(  empty($adw_order)) {
                ...
                if($status === $container->getParameter('nothrough')){
                    ...
                    if(! $processor->hasCertified($userId,$adid,$ocd,$v[5], $cps_advertisement)){
                        $msg .= '插入已认证数据失败';
                        $output->writeln('<error>'.$msg.'</error>');
                        $code[] = $msg;
                        $stats['accept_failed']++;
        ...
        if('prod' === $env) {
            ...
            $logger->info( 'file '. $csv_file . ' import results: '); 
            $logger->info( '总计:                '.($i - 2) );
            $logger->info( '插入拒绝数据失败:   '. $stats['refused_failed']);
            $logger->info( '插入已认证数据失败: '. $stats['accept_failed']);
            $logger->info( '插入拒绝数据成功:   '. $stats['refused_done']);
            $logger->info( '插入已认证数据成功: '. $stats['accept_done']);
            $logger->info( '订单状态无效:       '. $stats['no_order']);
        }
