...
class WebpowerSignupSuccessCommand extends ContainerAwareCommand
{
    
    protected function configure()
    {
        $this->setName('webpower-mailer:signup-success')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if( $env != 'prod') {
            ...
            $msg .= ',email:'.$email. ',name:'.$name.',title:'.$title;
            $logger->info($msg);
        }
        ...
        $return = $delivery_service->singleEmail(  array('email'=>$email, 'name'=>$name,'title'=>$title ) );
        $logger->info( $return );
    }
}
