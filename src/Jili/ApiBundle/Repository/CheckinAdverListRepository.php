...
class CheckinAdverListRepository extends EntityRepository
{
     ...
     */
    public function showCheckinList($uid,  $dateNow = null)
    {
        if( is_null($dateNow)) {
            $dateNow = date('Y-m-d');
        }
        ...
        $sqlD = 'SELECT d.id 
            ...
            WHERE c.user_id = :uid AND  inter_space > 1 AND DATEDIFF(:dateNow, c.click_date) < d.inter_space 
        ...
        $sqlU = 'SELECT e.id 
            ...
            WHERE u.user_id = :uid AND   click_date = :dateNow';
