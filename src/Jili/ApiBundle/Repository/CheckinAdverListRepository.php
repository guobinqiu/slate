...
class CheckinAdverListRepository extends EntityRepository
{
	...
	public function showCheckinList($uid){
		$dateNow = date('Y-m-d');
		$sqlD = "select d.id from checkin_user_list c inner join checkin_adver_list d on c.open_shop_id = d.id where user_id = 91 and inter_space > 1 and DATEDIFF('".$dateNow."',c.click_date) < d.inter_space group by c.open_shop_id order by c.click_date desc ";
		$sql = "SELECT a.id as cid,a.inter_space,b.*
			FROM checkin_adver_list a left join advertiserment b on a.ad_id = b.id 
			WHERE a.id not in ($sqlD) 
			ORDER BY a.id desc";
