...
KE.event.ready(function() {
	...
	var createList = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			img.src = 'http://cache.hujiaobeibei.net/images/' + iconName;
			img.width = 16;
			img.height = 16;
	...
	var createView = function(responseText) {
		...
		for (var i = 0, len = fileList.length; i < len; i++) {
			...
			cell.align = 'center';
			var fileUrl = result.current_url + data.filename;
			var iconUrl = data.is_dir ? 'http://cache.hujiaobeibei.net/images/folder-64.gif' : (data.is_photo ? fileUrl : 'http://cache.hujiaobeibei.net/images/file-64.gif');
			var img = KE.$$('img', document);
			img.src = iconUrl;
