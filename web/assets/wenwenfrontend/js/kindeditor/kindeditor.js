				...
				if (inputs.length > 0) {
					for (var i = 0, len = inputs.length; i < len; i++) {
						...
						if (btn && btn.type.toLowerCase() == 'button'){
							if (document.all) {
								btn.attachEvent("onmousedown", function(){KE.util.setData(id);});
							} else {
								btn.addEventListener('mousedown', function(){KE.util.setData(id);}, false);
							}
						}
					}
				}
...
var uploadRespData = null;

function activityContentPic(jsonStr) {
	uploadRespData = eval('(' + jsonStr + ')');
}