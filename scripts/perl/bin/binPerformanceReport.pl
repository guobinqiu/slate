...
$buff = $buff.sprintf "New: Newly registered users (including not active) within 30 days (+). \n=> %d\n", $newly_registered_user->register_number;
$buff = $buff.sprintf "Dead Dog: Newly registered users who have NOT earned any activation points (-). \n=> %d\n", $inactive_number;
$buff = $buff.sprintf "WIthdraw: Withdrawn within 30 days (-). \n=> %d\n", $withdraw_number;
$buff = $buff.sprintf "Forced to black List (-) \n=> %d\n", $delete_number;
$buff = $buff.sprintf "Late Active: Those who had registered 31 days ago but earned some points within 30 days (+) \n=> %d\n", $late_active_number;
